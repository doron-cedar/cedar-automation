...
async function createEmailAddress(page: Page, email: string, devSessionToken: string): Promise<string> {
    const createSignUpUrl = `${CLERK_STAGING_API}/v1/client/sign_ups?_clerk_dev_session=${devSessionToken}`;
    logger.info(`Attempting to create email address: ${email}`);
    logger.info(`Sending POST request to: ${createSignUpUrl}`);

    const response = await page.request.post(createSignUpUrl, {
        data: {
            email_address: email,
            strategy: 'email_code',
        },
